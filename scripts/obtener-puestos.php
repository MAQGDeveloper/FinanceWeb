...
//Se consulta la tabla catalogo_perfil y se obtienen los datos
$statement = $sql_conn->prepare("SELECT * FROM catalogo_perfil");
$statement->execute();
...
$catalogo_perfil = $statement->fetchAll(PDO::FETCH_ASSOC);
//Creo header de respuesta
header('Content-Type: application/json');
//Creo array con el mensaje de exito
$response = array(
    'status' => 200,
    'message' => $catalogo_perfil
);
//Codifico el array a json
$response = json_encode($response);
//Mando el json al cliente
